...
var _ repository.AuthRepository = &AuthRepository{}

type AuthRepository struct {
	conn *database.Conn
}

func NewAuthRepository(conn *database.Conn) repository.AuthRepository {
...
func (repo *AuthRepository) StoreToken(ctx context.Context, userId string, token *oauth2.Token) error {
	...
	query := `
	INSERT INTO github_auths (user_id, access_token, refresh_token, expiry)
